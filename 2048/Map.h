...
public:
	...
	bool search(Cl_keyType Cl_key, Cl_valueType Cl_value);
...
void ClMap<Cl_keyType, Cl_valueType>::insert(ClPair<Cl_keyType, Cl_valueType>& Cl_element)
{
	for (int i = 0; i < cPairs; i++)
		...
		{
			cout << "이미 존재하는 아이디와 비밀번호입니다." << endl;
			return;
		}
...
template <typename Cl_keyType, typename Cl_valueType>
bool ClMap<Cl_keyType, Cl_valueType>::search(Cl_keyType Cl_key, Cl_valueType Cl_value)
{
	for (ClIterator<Cl_keyType, Cl_valueType> Cl_it = begin(); Cl_it <= end(); Cl_it++)
	{
		if ((*Cl_it).first() == Cl_key)
		{
			if ((*Cl_it).second() == Cl_value)
			{
				return true;
			}
		}
